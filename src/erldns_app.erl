...
start(_Type, _Args) ->
    ...
    X = erldns_sup:start_link(),
    logger:notice("Supervisor of erldns started with:~n~p~n",[X]),
    X.
...
start_phase(post_start, _StartType, _PhaseArgs) ->
    ...
    logger:notice("Loading zones from local file.~n",[]),
    erldns_zone_loader:load_zones(),

    logger:notice("Notifying servers to start.~n",[]),
    erldns_events:notify({?MODULE, start_servers}),

    ok.

stop(_State) ->
    logger:notice("Stop erldns application.~n",[]),
    ok.
