...
handle_event({_M, start_servers}, State) ->
    case State#state.servers_running of
        false ->
            ...
            logger:notice("Starting the UDP and TCP supervisor",[]),
...
handle_event({M = erldns_zone_encoder, E = unsupported_rrdata_type, Data}, State) ->
    logger:notice("Unable to encode rrdata (module: ~p, event: ~p, data: ~p)", [M, E, Data]),
    {ok, State};
...
handle_event({M = erldns_zone_loader, E = put_zone_error, {JsonZone, Reason}}, State) ->
    logger:error("Failed to load zones (module: ~p, event: ~p, reason: ~p, json: ~p)", [M, E, Reason, JsonZone]),
    {ok, State};
...
handle_event({M = erldns_worker, E = restart_failed, Error}, State) ->
    logger:error("Restart failed (module: ~p, event: ~p, error: ~p)", [M, E, Error]),
    {ok, State};
