...
start(Address, Port, InetFamily) ->
    logger:notice("Starting UDP server (family: ~p, address: ~p, port: ~p)", [InetFamily, Address, Port]),
    case gen_udp:open(Port, [binary, {active, 100}, {reuseaddr, true}, {read_packets, 1000}, {ip, Address}, {recbuf, ?DEFAULT_UDP_RECBUF}, InetFamily]) of
        {ok, Socket} ->
            logger:notice("UDP server (family: ~p, address: ~p, socket: ~p)", [InetFamily, Address, Socket]),
            {ok, Socket};
        {error, eacces} ->
...
start(Address, Port, InetFamily, SocketOpts) ->
    logger:notice("Starting UDP server (family: ~p, address: ~p, port ~p, sockopts: ~p)", [InetFamily, Address, Port, SocketOpts]),
    ...
    of
        {ok, Socket} ->
            logger:notice("UDP server (family: ~p, address: ~p, socket: ~p, sockopts: ~p)", [InetFamily, Address, Socket, SocketOpts]),
            {ok, Socket};
        {error, eacces} ->
            logger:error("Failed to open UDP socket. Need to run as sudo?"),
            {error, eacces}
...
handle_request(Socket, Host, Port, Bin, State) ->
    case queue:out(State#state.workers) of
        ...
        {empty, _Queue} ->
            ...
            logger:notice("Queue is empty, dropping packet"),
            {noreply, State}
    end.
